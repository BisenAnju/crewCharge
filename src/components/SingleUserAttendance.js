...
class SingleUserAttendance extends Component {
  constructor(props) {
    super(props);
    this.state = {
      photoUrl: "",
  ...
  componentWillMount() {
    ...
    const userAttendance = this.props.attendanceData.filter(
      attendance => attendance.userId === this.props.match.params.id
    );
      ...
      <Layout navigationTitle="User Attendance" showBackNavigation={true}>
        ...
        >
          <LineChart
            width={600}
            height={350}
            data={this.state.userAttendance}
            margin={{
              top: 15,
              bottom: 5
            }}
